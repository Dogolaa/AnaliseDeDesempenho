...
int main(void){
    ...
    printf("---=== Bateria de 6 Cenários (Politica FCFS Global) ===---\n");
    ...
    for (int run = 0; run < NUM_RUNS; run++) {
        ...
        sprintf(nome_arquivo, "relatorio_fcfs_cenario_%s.csv", nomes_cenarios[run]);
        ...
        while(tempo_decorrido < tempo_simulacao){
            ...
            if(tipo_evento >= 0 && tipo_evento < NUM_FILAS) {
                ...
                // LÓGICA FCFS GLOBAL (O Mais Antigo)
                int fila_a_servir = -1;
